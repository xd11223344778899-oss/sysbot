      ...
      baseEmbed()
        ...
        .setDescription(
          [
            ...
            'نظام التفعيل: العضو الجديد يأخذ رول Unverified حتى يُفعَّل يدوياً أو بالرياكشن.',
            '',
            `\`${p}verify @عضو\` (أو \`فعل\`) — تفعيل عضو يدوياً.`,
            `\`${p}verifyall\` (أو \`فعلالجميع\`) — تفعيل كل الأعضاء غير المفعّلين دفعة واحدة.`,
            `\`${p}setverify\` — تفعيل نظام التحقق وتطبيق صلاحيات القنوات.`,
            `\`${p}setverifyreact <إيموجي>\` — تفعيل تلقائي بالرياكشن (رد على رسالة في قناة التفعيل).`,
            `\`${p}unsetverifyreact\` — إيقاف التفعيل بالرياكشن فقط.`,
            `\`${p}unsetverify\` — تعطيل نظام التحقق بالكامل.`,
            `\`${p}setchannel new #قناة\` / \`${p}setchannel verify #قناة\` / \`${p}setchannel prison #قناة\` / \`${p}setchannel adminlounge #قناة\` (شات إداري موحّد لموافقة السحب الصوتي).`,
            `\`${p}setnew <أيام>\` — تفعيل نظام الحسابات الجديدة.`,
            `\`${p}unsetnew\` — تعطيل نظام new.`,
